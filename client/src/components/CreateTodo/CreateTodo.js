...
const CreateTodo = ({ onClose, setAllTodos }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.post(`${FRONTEND_URL}/todo/createTask`, todo,
        {
          headers: {
  ...
  return (
    <>
      <BackgroundBlur>
        <OutsideClickHandler handleClose={() => onClose()}>
          <div className={styles.createTodoContainer}>
            ...
            {openCalendar &&
              <BackgroundBlur>
                <OutsideClickHandler handleClose={() => setOpenCalendar(false)}>
